...
class MyWidget(Widget):
    ...
    def on_touch_down(self, touch):
        with self.canvas:
            if self.erase is True:
                ...
                if self.cutFrame is True and len(self.frameList) != 0:
                    for index in self.frameList:
    ...
    def on_touch_move(self, touch):
        ...
        if self.cut is True:
            ...
            with self.canvas:
                Frame = Line(
                    points=[touch.opos[0], touch.opos[1], touch.x, touch.opos[1], touch.x, touch.y, touch.opos[0], touch.y,
                            touch.opos[0], touch.opos[1]], width=1, dash_length=4, dash_offset=1)
                self.frameList.append(Frame)

                if abs(self.list[0][0]) != abs(self.list[1][0]) or abs(self.list[0][1]) != abs(self.list[1][1]):
                    for index in self.frameList:
                        self.canvas.remove(index)
                        self.frameList.remove(index)
                    self.list.pop(0)
                    Frame = Line(
                        points=[touch.opos[0], touch.opos[1], touch.x, touch.opos[1], touch.x, touch.y, touch.opos[0],
                                touch.y,
                                touch.opos[0], touch.opos[1]], width=1, dash_length=4, dash_offset=1)
                    self.frameList.append(Frame)
